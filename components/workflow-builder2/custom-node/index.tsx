...
function CustomNode(props: NodeProps<CustomNodeType>) {
  ...
  return (
    <div 
        className={`text-updater-node border cursor-pointer bg-white hover:bg-gray-100 hover:shadow-lg transition-all shadow p-1 ${selected && 'ring-4 ring-red-400'}`} 
    ...
    >
      ...
      <div className='w-full space-y-0.5 flex flex-col items-center justify-center' >
          ...
          <div className='flex py-1 px-1 bg-gray-100 rounded-sm items-center gap-2 w-full h-6 ' >
            {data.label}
      ...
      {hasChild ? (
        ...
        <>
          <Handle
            ...
            className='relative'
          >
            {/* <div className='h-9 border border-black absolute left-1/2 -translate-x-1/2 top-[6px] ' /> */}

            
            {/* <button onClick={handleAddNode} className=' absolute   w-6 h-6 flex items-center justify-center rounded left-1/2 -translate-x-1/2 translate-y-10 bg-white hover:bg-gray-200 text-gray-700 border border-gray-700 '  ><BiPlus className='w-5 h-5'/></button> */}
          </Handle>
        </>
      )}
    </div>
