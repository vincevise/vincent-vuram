...
const getResponsiveColClass = (col: number) => {
  if (col === 6) return 'col-span-1 sm:col-span-2 md:col-span-6';
  return 'col-span-1 sm:col-span-1 md:col-span-3';
};
...
const ItemContainer = ({ col = 3, children, height, title, pointerEventNone, onClick, href }: Props) => {
  ...
  return (
    <div
      className={`bg-white  overflow-hidden flex items-center justify-center text-center ${!height && 'aspect-square'}  contianer-border-style relative group ${getResponsiveColClass(col)}`}
      style={{
        height,
      }}
    >
